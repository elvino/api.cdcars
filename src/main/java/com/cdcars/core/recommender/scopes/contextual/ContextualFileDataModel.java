 ...
 * {@link ContextualFileDataModel} without re-copying the entire data file.
 * </p>
 *
 * <p>
 ...
 */
public class ContextualFileDataModel extends AbstractDataModel {

  private static final Logger log = LoggerFactory.getLogger(ContextualFileDataModel.class);

  public static final long DEFAULT_MIN_RELOAD_INTERVAL_MS = 60 * 1000L; // 1 minute?
  private static final char COMMENT_CHAR = '#';
  private static final char[] DELIMIETERS = {',', '\t'};
  private static final String DELIMITERCONTEXTINFO = "\\|";

  private final File dataFile;
  private final ZipEntry entryZipFile;
  ...
  public ContextualFileDataModel(ZipEntry entry, ZipFile zip) throws IOException {

	  this.entryZipFile = entry;
	  this.zipFile = zip;
	  this.dataFile = null;
	  this.transpose = false;
	  this.minReloadIntervalMS = -1L;
	  
	    if (entry == null) {
	      throw new FileNotFoundException(zip.getName());
	    }

	    log.info("Creating ZipDataModel for file {}", entry.toString());
   ...
   */
  public ContextualFileDataModel(File dataFile) throws IOException {
   ...
   */
  public ContextualFileDataModel(File dataFile, String delimiterRegex) throws IOException {
   ...
   */
  public ContextualFileDataModel(File dataFile, boolean transpose, long minReloadIntervalMS) throws IOException {
   ...
   * @throws IOException
   */
  public ContextualFileDataModel(File dataFile, boolean transpose, long minReloadIntervalMS, String delimiterRegex)
    throws IOException {

	  this.entryZipFile = null;
	  this.zipFile = null;
  ...
  protected void reloadZip() {
	    if (reloadLock.tryLock()) {
	      try {
	        delegate = buildModelZip();
	      } catch (IOException ioe) {
	        log.warn("Exception while reloading", ioe);
	      } finally {
	        reloadLock.unlock();
  ...
  protected DataModel buildModelZip() throws IOException {
	    ...
	    if (hasPrefValues) {
	        ...
	        if (entryZipFile == null) {
			      throw new FileNotFoundException(zipFile.getName());
			 }
	        
	       // Scanner sc = new Scanner(entryZipFile);
	        
	        InputStreamReader streamReader = new InputStreamReader(this.zipFile.getInputStream(this.entryZipFile));
			BufferedReader reader = new BufferedReader(streamReader);
		    
	        processFileZip(reader, data, timestamps, false);
  ...
  protected DataModel buildModel() throws IOException {
    ...
    if (hasPrefValues) {
      ...
      if (loadFreshData) {
        ...
        FastByIDMap<PreferenceArray> rawData = ((ContextualDataModel) delegate).getRawUserData();
        ...
        return new ContextualDataModel(rawData, timestamps);
  ...
  protected void processFileZip(BufferedReader reader,
			FastByIDMap<?> data, FastByIDMap<FastByIDMap<Long>> timestamps, boolean fromPriorData) throws IOException {
		log.info("Reading zip file info...");
		int count = 0;
  ...
  protected void processLine(String line,
	    ...
	    boolean hasContextualPreference = tokens.hasNext();
	    String contextualPreferenceString = hasContextualPreference ? tokens.next() : null;
	    ...
	    long contextualPreferences[] = readContextualPreferencesFromString(contextualPreferenceString);
	    ...
	    if (fromPriorData) {
	     ...
	     ContextualUserPreferenceArray prefs = (ContextualUserPreferenceArray) maybePrefs;
	      if (!hasTimestamp && preferenceValueString.isEmpty()) {
	        ...
	        if (prefs != null) {
	          ...
	          if (exists) {
	            if (length == 1) {
	              ...
	              PreferenceArray newPrefs = new ContextualUserPreferenceArray(length - 1);
	        ...
	        if (!exists) {
	          if (prefs == null) {
	            prefs = new ContextualUserPreferenceArray(1);
	          } else {
	        	  ContextualUserPreferenceArray newPrefs = new ContextualUserPreferenceArray(prefs.length() + 1);
	          ...
	          prefs.setContextualPreferences(0, contextualPreferences);
	      ...
	      if (!hasTimestamp && preferenceValueString.isEmpty()) {
	        ...
	        if (!exists) {
	          ...
	          prefs.add(new ContextualPreference(userID, itemID, preferenceValue, contextualPreferences));
	        }
  ...
  protected long[] readContextualPreferencesFromString(String value) {
	  
	  
	  long contextualPreferences[]={};
	  
	  if(value!= null){
		  String splittedValue[] = value.split(DELIMITERCONTEXTINFO);
		  contextualPreferences = new long[splittedValue.length];
		  
		  for(int i=0; i < splittedValue.length; i++){
			  contextualPreferences[i] = new Long(splittedValue[i]);
		  }
  ...
  /**
   * Note that this method only updates the in-memory preference data that this {@link ContextualFileDataModel}
  ...
  public String toString() {
    return "ContextualFileDataModel[dataFile:" + dataFile + ']';
  }
