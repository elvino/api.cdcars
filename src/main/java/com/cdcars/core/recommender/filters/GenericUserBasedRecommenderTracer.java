...
import com.cdcars.core.recommender.scopes.contextual.ContextualPreferenceInterface;
import com.cdcars.core.recommender.scopes.item.ItemDomain;
import com.cdcars.core.recommender.evaluation.PredictionValues;
import com.cdcars.core.shared.util.ItemResourceUtil;
import com.cdcars.core.shared.util.UsersPreferencesUtil;
...
public class GenericUserBasedRecommenderTracer extends AbstractRecommender implements UserBasedRecommender {
	...
	private HashMap<Long, Integer> convertHashMap(ArrayList<ItemResourceUtil> sourceItens, int limitItens){
		
		HashMap<Long, Integer>  convertMap = new HashMap<Long, Integer>();
		
		for (int i = 0; i < sourceItens.size(); i++) {
			
			ItemResourceUtil is = sourceItens.get(i);
